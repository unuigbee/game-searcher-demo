...
open class DictionaryDecoder {
	...
	open func decode<T: Decodable>(_ type: T.Type, from dictionary: NSDictionary) throws -> T {
		let decoder = _DictionaryDecoder(referencing: dictionary, options: options)
...
private class _DictionaryDecoder: Decoder {
	...
	fileprivate init(referencing container: Any, at codingPath: [CodingKey] = [], options: DictionaryDecoder._Options) {
		storage = _DictionaryDecodingStorage()
...
private struct DictionaryCodingKeyedDecodingContainer<K: CodingKey>: KeyedDecodingContainerProtocol {
	...
	internal func notFoundError(key: Key) -> DecodingError {
		DecodingError.keyNotFound(key, DecodingError.Context(codingPath: decoder.codingPath, debugDescription: "No value associated with key \(_errorDescription(of: key))."))
	}

	internal func nullFoundError<T>(type: T.Type) -> DecodingError {
		DecodingError.valueNotFound(type, DecodingError.Context(codingPath: decoder.codingPath, debugDescription: "Expected \(type) value but found null instead."))
	}
	...
	public func nestedContainer<NestedKey>(keyedBy type: NestedKey.Type, forKey key: Key) throws -> KeyedDecodingContainer<NestedKey> {
		...
		let container = DictionaryCodingKeyedDecodingContainer<NestedKey>(referencing: decoder, wrapping: dictionary)
		return KeyedDecodingContainer(container)
	}
	...
	public func nestedUnkeyedContainer(forKey key: Key) throws -> UnkeyedDecodingContainer {
		...
		return _DictionaryUnkeyedDecodingContainer(referencing: decoder, wrapping: array)
	}
	...
	private func _superDecoder(forKey key: CodingKey) throws -> Decoder {
		...
		return _DictionaryDecoder(referencing: value, at: decoder.codingPath, options: decoder.options)
	}
...
private struct _DictionaryUnkeyedDecodingContainer: UnkeyedDecodingContainer {
	...
	public mutating func decodeNil() throws -> Bool {
		guard !isAtEnd else {
			throw DecodingError.valueNotFound(Any?.self, DecodingError.Context(codingPath: decoder.codingPath + [DictionaryCodingKey(index: currentIndex)], debugDescription: "Unkeyed container is at end."))
		}
	...
	public mutating func decode(_ type: Bool.Type) throws -> Bool {
		guard !isAtEnd else {
			throw DecodingError.valueNotFound(type, DecodingError.Context(codingPath: decoder.codingPath + [DictionaryCodingKey(index: currentIndex)], debugDescription: "Unkeyed container is at end."))
		}

		decoder.codingPath.append(DictionaryCodingKey(index: currentIndex))
		defer { self.decoder.codingPath.removeLast() }

		guard let decoded = try decoder.unbox(container[currentIndex], as: Bool.self) else {
			throw DecodingError.valueNotFound(type, DecodingError.Context(codingPath: decoder.codingPath + [DictionaryCodingKey(index: currentIndex)], debugDescription: "Expected \(type) but found null instead."))
		}
...
private extension _DictionaryDecoder {
	...
	func unbox(_ value: Any, as type: Bool.Type) throws -> Bool? {
		...
		if let number = value as? NSNumber {
			...
			/* TODO: If swift-corelibs-foundation doesn't change to use NSNumber, this code path will need to be included and tested:
	...
	func unbox(_ value: Any, as type: Int.Type) throws -> Int? {
		...
		guard NSNumber(value: int) == number else {
			throw DecodingError.dataCorrupted(DecodingError.Context(codingPath: codingPath, debugDescription: "Parsed Dictionary number <\(number)> does not fit in \(type)."))
		}
	...
	func unbox(_ value: Any, as type: UUID.Type) throws -> UUID? {
		guard !(value is NSNull) else { return nil }
	...
	func unbox<T: Decodable>(_ value: Any, as type: T.Type) throws -> T? {
		if type == Date.self || type == NSDate.self {
			...
			return try unbox(value, as: UUID.self) as? T
