// Disclaimer: Modified from https://github.com/ReactiveX/RxSwift/blob/main/RxCocoa/Runtime/_RXObjCRuntime.m
...
@class CombineObjCRuntime;

BOOL CombineAbortOnThreadingHazard = NO;
...
typedef BOOL (^CombineInterceptWithOptimizedObserver)(CombineObjCRuntime * __nonnull self, Class __nonnull class, SEL __nonnull selector, NSErrorParam error);
...
#define THREADING_HAZARD(class) \
    NSLog(@"There was a problem swizzling on `%@`.\nYou have probably two libraries performing swizzling in runtime.\nWe didn't want to crash your program, but this is not good ...\nYou an solve this problem by either not using swizzling in this library, removing one of those other libraries, or making sure that swizzling parts are synchronized (only perform them on main thread).\nAnd yes, this message will self destruct when you clear the console, and since it's non deterministic, the problem could still exist and it will be hard for you to reproduce it.", NSStringFromClass(class)); ABORT_IN_DEBUG if (CombineAbortOnThreadingHazard) { abort(); }

#define ALWAYS(condition, message) if (!(condition)) { [NSException raise:@"Combine Invalid Operator" format:@"%@", message]; }
...
#define Combine_PREFIX @"_Combine_namespace_"

#define Combine_ARG_id(value)           ((value) ?: [NSNull null])
#define Combine_ARG_char(value)         [NSNumber numberWithChar:value]
#define Combine_ARG_short(value)        [NSNumber numberWithShort:value]
#define Combine_ARG_int(value)          [NSNumber numberWithInt:value]
#define Combine_ARG_long(value)         [NSNumber numberWithLong:value]
#define Combine_ARG_BOOL(value)         [NSNumber numberWithBool:value]
#define Combine_ARG_SEL(value)          [NSNumber valueWithPointer:value]
#define Combine_ARG_rx_uchar(value)     [NSNumber numberWithUnsignedInt:value]
#define Combine_ARG_rx_ushort(value)    [NSNumber numberWithUnsignedInt:value]
#define Combine_ARG_rx_uint(value)      [NSNumber numberWithUnsignedInt:value]
#define Combine_ARG_rx_ulong(value)     [NSNumber numberWithUnsignedLong:value]
#define Combine_ARG_rx_block(value)     ((id)(value) ?: [NSNull null])
#define Combine_ARG_float(value)        [NSNumber numberWithFloat:value]
#define Combine_ARG_double(value)       [NSNumber numberWithDouble:value]
...
NSString * __nonnull const CombineObjCRuntimeErrorDomain   = @"CombineObjCRuntimeErrorDomain";
NSString * __nonnull const CombineObjCRuntimeErrorIsKVOKey = @"CombineObjCRuntimeErrorIsKVOKey";

BOOL Combine_return_type_is_supported(const char *type) {
    if (type == nil) {
        return NO;
    }
...
static BOOL Combine_method_has_supported_return_type(Method method) {
    ...
    return Combine_return_type_is_supported(methodSignature.methodReturnType);
}

SEL __nonnull Combine_selector(SEL __nonnull selector) {
    NSString *selectorString = NSStringFromSelector(selector);
    return NSSelectorFromString([Combine_PREFIX stringByAppendingString:selectorString]);
}
...
id __nonnull Combine_extract_argument_at_index(NSInvocation * __nonnull invocation, NSUInteger index) {
...
NSArray *Combine_extract_arguments(NSInvocation *invocation) {
    ...
    for (NSUInteger index = HIDDEN_ARGUMENT_COUNT; index < numberOfArguments; ++index) {
        [arguments addObject:Combine_extract_argument_at_index(invocation, index) ?: [NSNull null]];
    }
...
static BOOL Combine_forward_invocation(id __nonnull __unsafe_unretained self, NSInvocation *invocation) {
    SEL originalSelector = Combine_selector(invocation.selector);

    id<CombineMessageSentObserver> messageSentObserver = objc_getAssociatedObject(self, originalSelector);

    if (messageSentObserver != nil) {
        NSArray *arguments = Combine_extract_arguments(invocation);
...
static BOOL Combine_responds_to_selector(id __nonnull __unsafe_unretained self, SEL selector) {
...
static NSMethodSignatureRef Combine_method_signature(id __nonnull __unsafe_unretained self, SEL selector) {
    Class class = object_getClass(self);
...
static NSString * __nonnull Combine_method_encoding(Method __nonnull method) {
...
+(CombineObjCRuntime*)instance;

-(void)performLocked:(void (^)(CombineObjCRuntime* __nonnull))action;
...
+(void)registerOptimizedObserver:(CombineInterceptWithOptimizedObserver)registration encodedAs:(SEL)selector;

@end

/**
 All API methods perform work on locked instance of `CombineObjCRuntime`. In that way it's easy to prove
 that every action is properly locked.
 */
IMP __nullable Combine_ensure_observing(id __nonnull target, SEL __nonnull selector, NSErrorParam error) {
    ...
    @synchronized(target) {
        ...
        @synchronized([target class]) {
            [[CombineObjCRuntime instance] performLocked:^(CombineObjCRuntime * __nonnull self) {
...
#define FORWARD_BODY(invocation)                        if (Combine_forward_invocation(self, NAME_CAT(_, 0, invocation))) { return; }

#define RESPONDS_TO_SELECTOR_BODY(selector)             if (Combine_responds_to_selector(self, NAME_CAT(_, 0, selector))) return YES;

#define CLASS_BODY(...)                                 return actAsClass;

#define METHOD_SIGNATURE_FOR_SELECTOR_BODY(selector)                                            \
    NSMethodSignatureRef methodSignature = Combine_method_signature(self, NAME_CAT(_, 0, selector)); \
    if (methodSignature != nil) {                                                               \
        return methodSignature;                                                                 \
    }

#define DEALLOCATING_BODY(...)                                                        \
    id<CombineDeallocatingObserver> observer = objc_getAssociatedObject(self, rxSelector); \
    if (observer != nil && observer.targetImplementation == thisIMP) {                \
        [observer deallocating];                                                      \
    }

#define OBSERVE_BODY(...)                                                              \
    id<CombineMessageSentObserver> observer = objc_getAssociatedObject(self, rxSelector);   \
                                                                                       \
    if (observer != nil && observer.targetImplementation == thisIMP) {                 \
...
#define BUILD_ARG_WRAPPER(type)                   Combine_ARG_ ## type                                                     //Combine_ARG_ ## type

#define CAT(_1, _2, head, tail)                   Combine_CAT2(head, tail)
#define SEPARATE_BY_COMMA(_1, _2, head, tail)     head, tail
#define SEPARATE_BY_SPACE(_1, _2, head, tail)     head tail
#define SEPARATE_BY_UNDERSCORE(head, tail)        Combine_CAT2(Combine_CAT2(head, _), tail)

#define UNDERSCORE_TYPE_CAT(_1, index, type)      Combine_CAT2(_, type)                                                    // generates -> _type
...
#define EXAMPLE_PARAMETER(_1, index, type)        Combine_CAT2(_, type):(type)SEPARATE_BY_UNDERSCORE(type, index)          // generates -> _type:(type)type_0
#define SELECTOR_PART(_1, index, type)            Combine_CAT2(_, type:)                                                   // generates -> _type:

#define COMMA_DELIMITED_ARGUMENTS(...)            Combine_FOREACH(_, SEPARATE_BY_COMMA, NOT_NULL_ARGUMENT_CAT, ## __VA_ARGS__)
#define ARGUMENTS(...)                            Combine_FOREACH_COMMA(_, NAME_CAT, ## __VA_ARGS__)
#define DECLARE_ARGUMENTS(...)                    Combine_FOREACH_COMMA(_, TYPE_AND_NAME_CAT, ## __VA_ARGS__)

// optimized observe methods

#define GENERATE_SELECTOR_IDENTIFIER(...)         Combine_CAT2(exampleSelector, Combine_FOREACH(_, CAT, UNDERSCORE_TYPE_CAT, ## __VA_ARGS__))

#define GENERATE_METHOD_IDENTIFIER(...)           Combine_CAT2(swizzle, Combine_FOREACH(_, CAT, UNDERSCORE_TYPE_CAT, ## __VA_ARGS__))
...
#define BUILD_EXAMPLE_METHOD(return_value, ...) \
    +(return_value)Combine_CAT2(Combine_CAT2(example_, return_value), Combine_FOREACH(_, SEPARATE_BY_SPACE, EXAMPLE_PARAMETER, ## __VA_ARGS__)) {}

#define BUILD_EXAMPLE_METHOD_SELECTOR(return_value, ...) \
    Combine_CAT2(Combine_CAT2(example_, return_value), Combine_FOREACH(_, SEPARATE_BY_SPACE, SELECTOR_PART, ## __VA_ARGS__))
...
#define SWIZZLE_OBSERVE_METHOD_BODY(return_value, ...)                                                                                                       \
       ...
       [self registerOptimizedObserver:^BOOL(CombineObjCRuntime * __nonnull self, Class __nonnull class,                                                     \
...
method_prototype                                                                                                         \
    __unused SEL rxSelector = Combine_selector(selector);                                                                     \
...
@interface CombineObjCRuntime (InfrastructureMethods)
@end

// MARK: Infrastructure Methods

@implementation CombineObjCRuntime (InfrastructureMethods)
...
@interface CombineObjCRuntime (swizzle)

@end

@implementation CombineObjCRuntime(swizzle)
...
// MARK: CombineObjCRuntime

@implementation CombineObjCRuntime

static CombineObjCRuntime *_instance = nil;
static NSMutableDictionary<NSString *, CombineInterceptWithOptimizedObserver> *optimizedObserversByMethodEncoding = nil;
...
+(void)initialize {
    _instance = [[CombineObjCRuntime alloc] init];
    defaultTypeID = CFGetTypeID((CFTypeRef)CombineObjCRuntime.class); // just need a reference of some object not from CF
...
+(void)registerOptimizedObserver:(CombineInterceptWithOptimizedObserver)registration encodedAs:(SEL)selector {
    ...
    NSString *methodEncoding = Combine_method_encoding(exampleEncodingMethod);
...
-(IMP __nullable)ensurePrepared:(id __nonnull)target forObserving:(SEL __nonnull)selector error:(NSErrorParam)error {
    ...
    if (instanceMethod == nil) {
        Combine_THROW_ERROR([NSError errorWithDomain:CombineObjCRuntimeErrorDomain
                                           code:CombineObjCRuntimeErrorSelectorNotImplemented
                                       userInfo:nil], nil);
    }
    ...
    ||  selector == @selector(respondsToSelector:)) {
        Combine_THROW_ERROR([NSError errorWithDomain:CombineObjCRuntimeErrorDomain
                                           code:CombineObjCRuntimeErrorObservingPerformanceSensitiveMessages
                                       userInfo:nil], nil);
    }
    ...
    if (selector == deallocSelector) {
        ...
        NSString *methodEncoding = Combine_method_encoding(instanceMethod);
        CombineInterceptWithOptimizedObserver optimizedIntercept = optimizedObserversByMethodEncoding[methodEncoding];

        if (!Combine_method_has_supported_return_type(instanceMethod)) {
            Combine_THROW_ERROR([NSError errorWithDomain:CombineObjCRuntimeErrorDomain
                                               code:CombineObjCRuntimeErrorObservingMessagesWithUnsupportedReturnType
                                           userInfo:nil], nil);
        }
        ...
        // default fallback to observing by forwarding messages
            ...
            if ([self forwardingSelector:selector forClass:swizzlingImplementorClass]) {
                return Combine_default_target_implementation();
            }
    ...
    Combine_THROW_ERROR([NSError errorWithDomain:CombineObjCRuntimeErrorDomain
                                       code:CombineObjCRuntimeErrorUnknown
                                   userInfo:nil], nil);
}
...
-(Class __nullable)prepareTargetClassForObserving:(id __nonnull)target error:(NSErrorParam)error {
    ...
    if (isThisTollFreeFoundationClass) {
        Combine_THROW_ERROR([NSError errorWithDomain:CombineObjCRuntimeErrorDomain
                                           code:CombineObjCRuntimeErrorCantInterceptCoreFoundationTollFreeBridgedObjects
                                       userInfo:nil], nil);
    }
     ...
     * `sentMessage` sets object class to be `_Combine_namespace___KVO__OriginalClass` (subclass of `__KVO__OriginalClass`)
    ...
    if ([target class] != object_getClass(target)) {
        ...
        Combine_THROW_ERROR([NSError errorWithDomain:CombineObjCRuntimeErrorDomain
                                           code:CombineObjCRuntimeErrorObjectMessagesAlreadyBeingIntercepted
                                       userInfo:@{
                                                  CombineObjCRuntimeErrorIsKVOKey : @(isKVO)
                                                  }], nil);
    }
    ...
    if (previousClass != wannaBeClass) {
        ...
        Combine_THROW_ERROR([NSError errorWithDomain:CombineObjCRuntimeErrorDomain
                                           code:CombineObjCRuntimeErrorThreadingCollisionWithOtherInterceptionMechanism
                                       userInfo:nil], nil);
    }
...
-(BOOL)observeByForwardingMessages:(Class __nonnull)swizzlingImplementorClass
...
#endif
    SEL rxSelector = Combine_selector(selector);
    ...
    if (!class_addMethod(swizzlingImplementorClass, rxSelector, implementation, methodEncoding)) {
        Combine_THROW_ERROR([NSError errorWithDomain:CombineObjCRuntimeErrorDomain
                                           code:CombineObjCRuntimeErrorSavingOriginalForwardingMethodFailed
                                       userInfo:nil], NO);
    }
    ...
    if (!class_addMethod(swizzlingImplementorClass, selector, _objc_msgForward, methodEncoding)) {
        if (implementation != method_setImplementation(instanceMethod, _objc_msgForward)) {
            ...
            Combine_THROW_ERROR([NSError errorWithDomain:CombineObjCRuntimeErrorDomain
                                               code:CombineObjCRuntimeErrorReplacingMethodWithForwardingImplementation
                                           userInfo:nil], NO);
        }
    }
...
-(Class __nullable)ensureHasDynamicFakeSubclass:(Class __nonnull)class error:(NSErrorParam)error {
    ...
    NSString *dynamicFakeSubclassName = [Combine_PREFIX stringByAppendingString:NSStringFromClass(class)];
...
NSInteger Combine_number_of_dynamic_subclasses(void) {
...
NSInteger Combine_number_of_forwarding_enabled_classes(void) {
    __block NSInteger count = 0;
    [[CombineObjCRuntime instance] performLocked:^(CombineObjCRuntime * __nonnull self) {
...
NSInteger Combine_number_of_intercepting_classes(void) {
    __block NSInteger count = 0;
    [[CombineObjCRuntime instance] performLocked:^(CombineObjCRuntime * __nonnull self) {
...
NSInteger Combine_number_of_forwarded_methods(void) {
    return numberOfForwardedMethods;
}

NSInteger Combine_number_of_swizzled_methods(void) {
